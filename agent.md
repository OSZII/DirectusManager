...
- **Fields**: `id`, `name`, `url`, `token`, `gitRemoteUrl`, `gitToken`, `encryptedToken`, `encryptedGitToken`.

### 2. Directus Sync Operations
Uses `npx directus-sync` under the hood. For each instance, a directory is created in the app's `userData` folder.
